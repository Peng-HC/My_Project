...
std::vector<uint8_t> PolarCode::decode(double ebno_vec,uint8_t list_size_vec,std::vector<uint8_t> encode_msg) {
    ...
    coded_bits = encode_msg;

    double ebno = ebno_vec;
    uint8_t list_size = list_size_vec;

    double snr_sqrt_linear = std::pow(10.0f, ebno/20)
                             * std::sqrt( ((double) _info_length )/((double) (_block_length) )) ;

    for (uint16_t i = 0; i < _block_length; ++i) {
        llr.at(i) = - 4 * (2 * coded_bits.at(i) - 1) * std::pow(snr_sqrt_linear,2) / N_0;
    }
