...
void GobangServer::stop() {
    ...
    //shutdown 系统调用关闭连接的读数据通道、写数据通道或者读写数据通道；
    shutdown(socketfd, 2);
...
//启动服务器,监听客户端的socket连接
bool GobangServer::start(int port) {
    //AF_INET:IPV4 SOCK_STREAM:流socket(TCP)
    ...
    servaddr.sin_addr.s_addr = htonl(INADDR_ANY);//主机字节序转网络字节序(长整型,4字节)
    servaddr.sin_port = htons(port);//主机字节序转网络字节序(短整型,2字节)

    //将socket文件句柄与ip地址和端口绑定起来
